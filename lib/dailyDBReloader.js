...
const { BASE_DIR, DB_CURRENT_LOAD_FILE, DB_CURRENT_LOAD_FILE_URL, DB_DAILY_RELOAD_TIME } = {
...
dailyDBReloader.processFile = async () => {
	...
	loadSentiments.then(() => {
		console.log(
			`Loading database took ${((performance.now() - t0) / 60000).toFixed(4)} minutes to perform.`
		);
	});
};
...
dailyDBReloader.runLoop = () => {
	setInterval(() => {
		...
		timeToRetrieveData.setHours(DB_DAILY_RELOAD_TIME[0], DB_DAILY_RELOAD_TIME[1], 0);
